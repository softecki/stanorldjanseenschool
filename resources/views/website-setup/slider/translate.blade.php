...
@section('content')
    <div class="page-content">
        ...
        <div class="card ot-card">
            <div class="card-body">
                <form action="{{ route('slider.translate.update', @$data['slider']->id) }}" enctype="multipart/form-data" method="post"
                    id="visitForm">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                        ...
                                        <input class="form-control ot-input @error('name') is-invalid @enderror" name="name[{{$language->code}}]"
                                            value="{{isset($data['translates'][$language->code][0]) ? $data['translates'][$language->code][0]->name :  @$data['slider']->name}}" list="datalistOptions" id="exampleDataList"
                                    ...
                                    <div class="col-md-12 mt-3">
                                        ...
                                        placeholder="{{ ___('common.Enter description') }}">{{isset($data['translates'][$language->code][0]) ? $data['translates'][$language->code][0]->description :  @$data['slider']->description}}</textarea>
                                        @error('description')
                                            <div id="validationServer04Feedback" class="invalid-feedback">
                                                {{ $message }}
                                            </div>
