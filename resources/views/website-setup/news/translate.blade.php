...
@section('content')
    <div class="page-content">
        ...
        <div class="card ot-card">
            <div class="card-body">
                <form action="{{ route('news.translate.update', @$data['news']->id) }}" enctype="multipart/form-data" method="post"
                    id="visitForm">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                        ...
                                        <input class="form-control ot-input @error('title') is-invalid @enderror" name="title[{{$language->code}}]"
                                            value="{{isset($data['translates'][$language->code][0]) ? $data['translates'][$language->code][0]->title :  @$data['news']->title}}" list="datalistOptions" id="exampleDataList"
                                    ...
                                    <div class="col-md-12 mt-3">
                                        ...
                                        placeholder="{{ ___('common.Enter description') }}">{{isset($data['translates'][$language->code][0]) ? $data['translates'][$language->code][0]->description :  @$data['news']->description}}</textarea>
                                        @error('description')
                                            <div id="validationServer04Feedback" class="invalid-feedback">
                                                {{ $message }}
                                            </div>
