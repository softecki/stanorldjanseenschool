...
@section('content')
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <h1 class="bradecrumb-title mb-1">{{ ___('settings.fees_collection') }}</h1>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">{{ ___('common.home') }}</a></li>
                        <li class="breadcrumb-item">{{ ___('settings.fees_collection') }}</li>
                    </ol>
                </div>
            </div>
        </div>
...
{{--                                <a class="btn btn-md btn-outline-primary" href="{{ route('report-fees-collection.pdf-generate', ['class'=>$data['request']->class,'section'=>$data['request']->section, 'dates'=>Illuminate\Support\Facades\Crypt::encryptString($data['request']->dates)]) }}">--}}
{{--                                    {{___('common.pdf_download')}}--}}
{{--                                    <span><i class="fa-brands fa-dochub"></i></span>--}}
{{--                                </a>--}}

                                {{-- <a class="btn btn-md btn-outline-success" href="{{ route('report-fees-collection.excel-generate', ['class'=>$data['request']->class,'section'=>$data['request']->section, 'dates'=>Illuminate\Support\Facades\Crypt::encryptString($data['request']->dates)]) }}">
                                    {{'Export Excel'}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-border ">
                                ...
                                <tr style="font-size: 12px;">
                                    <th style="font-size: 12px;">{{___('common.#')}}</th>
                                    <th style="font-size: 12px;">{{___('common.name')}}</th>
                                    <th style="font-size: 12px;">{{___('academic.class')}} </th>
                                    <th style="font-size: 12px;">{{'Due 2024'}}</th>
                                    <th style="font-size: 12px;">{{'Due Term One'}}</th>
                                    <th style="font-size: 12px;">{{'Paid Term One'}}</th>
                                    <th style="font-size: 12px;">{{'Fee Amount'}}</th>
                                    <th style="font-size: 12px;">{{'Paid '}} </th>
                                    <th style="font-size: 12px;">{{'Remained '}} ({{ Setting('currency_symbol') }})</th>
                                ...
                                @forelse ($data['result'] as $key=>$item)
                                    <tr style="font-size: 12px;">
                                        ...
                                        <td style="font-size: 12px;">{{ $item->class_name }} </td>
                                        <td style="font-size: 12px;">{{ $item->outstanding_remained??'0' }}</td>
                                        <td style="font-size: 12px;">{{ ($item->quater_one + $item->quater_two) }}</td>
                                        <td style="font-size: 12px;">{{ ($item->fees_amount/2) - ($item->quater_one + $item->quater_two) }}</td>
                                        <td style="font-size: 12px;">{{ $item->fees_amount/2 }}</td>
                                        <td style="font-size: 12px;">{{ $item->paid_amount }}</td>
                                        <td style="font-size: 12px;">{{ $item->remained_amount  }}</td>
                                        <td style="font-size: 12px;"> @if($item->remained_amount > 0)
                                                <span class="text-danger">Unpaid</span>
                                            @elseif($item->remained_amount < 0)
                                                <span class="text-warning">Over paid</span>
