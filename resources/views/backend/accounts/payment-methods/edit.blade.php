...
<div class="page-content">
    ...
    <div class="card ot-card">
        <div class="card-body">
            <form action="{{ route('payment-methods.update', $data['method']->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label">{{ ___('common.name') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control ot-input @error('name') is-invalid @enderror" name="name" value="{{ old('name', $data['method']->name) }}" required>
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">{{ ___('common.description') }}</label>
                        <input type="text" class="form-control ot-input" name="description" value="{{ old('description', $data['method']->description) }}">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-md-6">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" {{ old('is_active', $data['method']->is_active) ? 'checked' : '' }}>
