...
@section('content')
    <div class="page-content">
        ...
        <div class="card ot-card">
            <div class="card-body">
                <form action="{{ route('storekeeper.sellupdate', @$data['stock']->id) }}" enctype="multipart/form-data" method="post"
                    id="visitForm">
                    @csrf
                    @method('PUT')
                    <div class="row mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input class="form-control ot-input @error('name') is-invalid @enderror" name="name"
                                        value="{{ old('name',@$data['stock']->name) }}" list="datalistOptions" id="exampleDataList"
                                ...
                                <div class="col-md-6">
                                    ...
                                    aria-describedby="validationServer04Feedback">
                                        ...
                                        <option value="{{ App\Enums\Status::ACTIVE }}"
                                            {{ @$data['stock']->status == App\Enums\Status::ACTIVE ? 'selected' : '' }}>
                                            {{ ___('common.active') }}</option>
                                        <option value="{{ App\Enums\Status::INACTIVE }}"
                                            {{ @$data['stock']->status == App\Enums\Status::INACTIVE ? 'selected' : '' }}>
                                            {{ ___('common.inactive') }}
                                        </option>
                                    </select>
                                </div>
