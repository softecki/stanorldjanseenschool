...
export function OnlineExamResultPage({ Layout }) {
    const { id } = useParams();
    const [data, setData] = useState(null);
    const [err, setErr] = useState('');

    useEffect(() => {
        axios.get(`/student-panel-online-examination/result-view/${id}`, { headers: xhrJson })
            .then((r) => setData(r.data?.data || {}))
            .catch((ex) => setErr(ex.response?.data?.message || 'Failed to load result.'));
    }, [id]);

    const entries = useMemo(() => toEntries(data), [data]);

    return (
        <Shell Layout={Layout} title="Online Exam Result">
            ...
            <div className="rounded border bg-white p-4">
                {entries.length ? (
                    ...
                    <p className="text-sm text-slate-500">No result detail available.</p>
                )}
            </div>
        </Shell>
    );
